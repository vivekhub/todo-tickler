...
RE_WEEKDAY = (
    "(daily)|(first|second|third|fourth|fifth)-(sunday|monday|"
...
def FindProjectsAndContexts(aline):
    ...
    duedates = [aword for aword in ticklerwords if RE_DUE_DATE.match(aword) is not None]

    task_id = [aword for aword in ticklerwords if task_id_re.match(aword) is not None]

    task_dep_ids = [
        aword for aword in ticklerwords if task_dep_ids_re.match(aword) is not None
    ]
    ActualDates = [
        datetime.strptime(adate.split(":")[1], "%Y-%m-%d") for adate in duedates
    ]
    ...
    return (
        ...
        set(contexts),
        set(ActualDates),
        task_id[0] if task_id else None,
        set(task_dep_ids),
    )
...
def RepeatDateInRange(start, delta, end=None):
    ...
    if end and end < sunday:
        return None
    saturday = (
        end if end else date.today() + timedelta((6 - date.today().isoweekday()) % 7)
    )
    if start <= date.today():
        check = start
        while check < sunday:
...
def ProcessTicklerFile(aFile, todoFile, TodoContent):
                ...
                if not MatchLineFuzzy(TodoContent, outwords):
                    ...
                    if len(start_date_list) > 0:
                        ...
                        if len(repeat_word) > 0:
                            keydates = ParseRepeat(startdate, repeat_word[0], enddate)
                            if keydates:
                                InsertTaskLine(
                                    todo, outwords, keydates[0], keydates[1], firsttime
                                )
                                firsttime = False
                                processed += 1
                            ...
                            enddate = (
                                ...
                                if duedates
                                else startdate + timedelta(days=7)
                            )
                            ...
                            if next_step == 1:
                                InsertTaskLine(
                                    todo, outwords, startdate, enddate, firsttime
                                )
                                firsttime = False
                                processed += 1
                            elif next_step == 0:
    ...
    print(
        "Added {0} item{1} from the tickler file".format(
...
if __name__ == "__main__":
    ...
    print(f'Total Todo items this week : {len(TodoFile[2])}')
